...
// QueryCreds chains the current query on the "creds" edge.
func (uq *UserQuery) QueryCreds() *CredsQuery {
	query := &CredsQuery{config: uq.config}
...
// WithCreds tells the query-builder to eager-load the nodes that are connected to
// the "creds" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithCreds(opts ...func(*CredsQuery)) *UserQuery {
	query := &CredsQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withCreds; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Creds = []*Creds{}
		}
		query.withFKs = true
		query.Where(predicate.Creds(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CredsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_creds
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_creds" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_creds" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Creds = append(node.Edges.Creds, n)
		}
	}
